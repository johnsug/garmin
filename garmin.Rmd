...
```{r fix list}
# update labels in changepoint table
# add points to benchmark plot
# explain confidence interval
# (do period/period on linear model? or just raw values?)
# change "value" label to something like "metric value"
# need to go back and bring running data from september onwards (make sure calendar heatmap remains 2016 only...)
# on trend page, mentions side-by-side comparison... 
# switch radios with dropboxes (to enable more months?)
```
...
# personal records
pr <- DF(fread("personal_records.csv"))
...
<h4>I am a runner and a data junkie, and really love analyzing my runs recorded by my [Garmin Forerunner 620](https://buy.garmin.com/en-US/US/wearabletech/wearables/previous_models_wearables/forerunner-620/prod122785.html) GPS watch and heart rate monitor. The [Garmin Connect](http://connect.garmin.com) dashboards that sync with my watch are great, however when I can't find the insights I want, I perform my own statistical modeling using my personal running data. This dashboard demonstrates some of the analytical views I've come up with:
  
* <a href=#changepoint-analysis>Changepoint Analysis</a> - evaluates changes in intensity during selected activities
* <a href=#pace-benchmarking>Pace Benchmarking</a> - compares recent runs to historical averages and records
* <a href=#calendar-heatmap>Calendar Heatmap</a> - quickly illustrates annual activity (annual runs)
...
shinyApp(
  ...
  ui = 
    fillPage(
      selectInput("changepoint_list", "Choose Activity", choices=dat$Full_Activity_Name, width=400),
      fluidRow(
        box(dygraphOutput("changepoint_plot", height="300px"), width=9),
  ...
  server = function(input, output) {
    ...
    cp_table_data <- reactive({
      ...
      names(td) <- c("Zone", "Minutes", "Distance (Miles)", "Average MPH", "Average Pace", 
                     "Average Heart Rate")
      td[td$Minutes!=0,]
    })
    ...
    output$changepoint_plot <- renderDygraph({
      ...
      dygraph(merged, xlab="Time in Hours : Minutes", ylab="Running Speed (MPH)") %>% 
...
to the values of a time series. Such techniques are commonly used to determine deviations in manufacturing processes, medical device measurements, and website traffic.

In this example, I'm using changepoint analysis to determine shifts in the intensity of recorded runs, in terms of instanteous speed. (While runners are usually more concerned about average pace, looking at their speed in terms of miles per hour makes more sense in this example.) Each segment is displayed visually, and the associated table details summary statistics from each significant shift in intensity.</h4>
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    output$mc_plot <- renderPlotly({
      ...
      ## plot points
      plot_ly(eff_dat(), x=Distance, y=Mins, name="Activity", hoverinfo="text", 
              text=paste0(Start, 
      ...
      add_trace(x=avg_pace()$x, y=avg_pace()$y, name="Average Pace", mode="lines", opacity=.3, 
                showlegend=T, line=list(width=5)) %>%
...
<h4>In this plot, historical activities (blue dots) are compared to benchmarks of both historical best paces (green line) and average paces (gold line) from the selected time horizon.</h4>
...
<h4>This widget, inspired by Github's productivity heatmap, can be used to quickly illusrates day-of-the-week and week-to-week activity trends.</h4>
...
shinyApp(
  ...
  server = function(input, output) {
    ...
    output$trend_horizon <- renderUI({
      ...
      ## return UI widget
      radioButtons("trend_horizon", label="Time horizon:", choices=flex_list, inline=T, selected=12)
    })
    ...
    gg_01 <- reactive({
      ...
      g1 <- ggplot(pd, aes(x=Date, y=Value)) + 
              ...
              stat_smooth(method="lm", col="blue", fill="royalblue", alpha=.3, fullrange=T) +
    ...
    gg_02 <- reactive({
      ...
      g2 <- ggplot(pd, aes(x=Date, y=Value)) + 
              ...
              geom_point(size=2) + 
              ## lm, glm, gam, rlm, loess, 
              stat_smooth(method="lm", col="blue", fill="royalblue", alpha=.3, fullrange=T) + #royalblue
              scale_x_date(date_breaks="2 months", date_labels="%b %d")
...
<h4>This widget provides simple trend analysis, highlighting period-over-period changes in the specified metrics as well as provides simple linear trend modeling.</h4>
