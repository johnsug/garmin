...
```{r heart rate plot}
ggplot(dat=a[!is.na(HR)], aes(x=Date, y=HR)) + 
  geom_line(col="gray") + 
  geom_point(size=2) + 
  stat_smooth(method="lm", fill="royalblue", alpha=.3, fullrange=T) + 
  scale_x_date(date_breaks="2 months", limits=c(range(a$Date, max(a$Date+30))), date_labels = "%b %d")
...
## need to filter dates!!

ggplot(dat=a, aes(x=Date, y=MPH)) + 
  geom_line(col="gray") + 
  geom_point(size=2) + 
  stat_smooth(method="lm", fill="royalblue", alpha=.3, fullrange=T) + 
  scale_x_date(date_breaks="2 months", limits=c(range(a$Date, max(a$Date+30))), date_labels = "%b %d")
